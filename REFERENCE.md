...
Preserved from the README at 0.7.0, during the 2026-09-16 documentation cleanup. This includes earlier release notes, detailed contracts, and historical validation claims. Earlier setup steps or limitations may have been superseded. Start with [the current README](README.md), current source, and the tagged release. Relative source links remain rooted in this repository.

---

## 0.7.0 — Device-local AI configuration

Provider preferences now use Obsidian’s vault-scoped device-local storage. Apple Intelligence and Ollama enablement, Ollama URL/model, cloud credential references/models, provider order, and diagnostics stay on this device. Controls are labeled This device. First load seeds local preferences from the previous configuration; existing local values always win afterward, including disabled options. Legacy shared settings remain as first-run defaults for other existing devices, but preference edits no longer write them. Secret values remain in SecretStorage. The three settings destinations and actions remain unchanged; no outbound diagnostic or model request runs during migration.

Verified all three reloaded settings destinations and their This device labels. A temporary local preference change survived settings reload while shared plugin data remained byte-equivalent; the original value was restored. Tests cover local snapshot precedence, including false toggles, and migration/persistence behavior. No provider requests were made. Cloud is still the default route; Local and Diagnostics remain one click away, with no added disclosure levels. Full declared npm test and a separate final npm run build are required for the versioned artifacts; the stable build deploys only shipped files to Obsidian Plugin Test Vault. Hot Reload plus explicit test-plugin reload was used for UI verification. Release notes record final validation and SHA-256 hashes. Production installation remains the user’s BRAT pull.

## 0.6.1 — cleaner settings copy
